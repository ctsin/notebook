...
- [Possible reason on Prettier and ESLint conflict](#possible-reason-on-prettier-and-eslint-conflict)
  ...
  - [In `rules` statement](#in-rules-statement)
- [Protected Route with React Route](#protected-route-with-react-route)
- [The difference between `for..of` and `for..in`](#the-difference-between-forof-and-forin)
- [Good advice on JSX conditionals](#good-advice-on-jsx-conditionals)
- [Format a list with `Intl`](#format-a-list-with-intl)
- [Unicode Segmentation in JavaScrip](#unicode-segmentation-in-javascrip)
- [Input emoji](#input-emoji)
- [Show hidden files in MacOS](#show-hidden-files-in-macos)
...
- [A snippet of `useContext`](#a-snippet-of-usecontext)
...
- [`useMemo` is not restricted to use on the most top of code](#usememo-is-not-restricted-to-use-on-the-most-top-of-code)
- [Debug setting for CRA in VS Code](#debug-setting-for-cra-in-vs-code)
- [`<input />` events tips](#input--events-tips)
- [Accessing `NODE_ENV`](#accessing-node_env)
- [VS Code IntelliSense](#vs-code-intellisense)
- [`<fieldset />`](#fieldset-)
- [Serving sharp images to high density screens](#serving-sharp-images-to-high-density-screens)
- [How to Dynamically Import ECMAScript Modules](#how-to-dynamically-import-ecmascript-modules)
- [ES2021](#es2021)
- [Optional chain tip](#optional-chain-tip)
- [The extended types can be narrower than its parent ones only](#the-extended-types-can-be-narrower-than-its-parent-ones-only)
- [3 Useful TypeScript Patterns to Keep in Your Back Pocket](#3-useful-typescript-patterns-to-keep-in-your-back-pocket)
  - [Pattern 1: Mapped Types 👍](#pattern-1-mapped-types-)
  - [Pattern 2: Function Overloading](#pattern-2-function-overloading)
  - [Pattern 3: Custom Type Guards](#pattern-3-custom-type-guards)
- [React uncontrolled input issue](#react-uncontrolled-input-issue)
- [Enums](#enums)
  - [Enums at compile time](#enums-at-compile-time)
  - [Reverse mappings](#reverse-mappings)
  - [Objects vs Enums](#objects-vs-enums)
- [TypeScript Config for `import` statement shortcut](#typescript-config-for-import-statement-shortcut)
- [Google layout demonstration](#google-layout-demonstration)
  - [Grid](#grid)
- [HTML Tips](#html-tips)
  - [The `loading=lazy` attribute](#the-loadinglazy-attribute)
  - [Email, call, and SMS links:](#email-call-and-sms-links)
  - [Ordered lists `start` attribute.](#ordered-lists-start-attribute)
  - [The `meter` element](#the-meter-element)
  - [HTML Native Search](#html-native-search)
  - [Fieldset Element](#fieldset-element)
  - [Base Element](#base-element)
  - [Favicon cache busting](#favicon-cache-busting)
  - [The `spellcheck` attribute](#the-spellcheck-attribute)
  - [Native HTML sliders](#native-html-sliders)
  - [HTML Accordion](#html-accordion)
  - [`mark` tag](#mark-tag)
  - [`download` attribute](#download-attribute)
  - [Performance tip](#performance-tip)
  - [input type="search"](#input-typesearch)
- [Journey of Improving React App Performance](#journey-of-improving-react-app-performance)
    - [Remove all Inline Functions](#remove-all-inline-functions)
    - [Conditional rendering of the components](#conditional-rendering-of-the-components)
    - [Remove unnecessary awaits and use Promise.all() wherever applicable](#remove-unnecessary-awaits-and-use-promiseall-wherever-applicable)
...
- [Styled Components Best Practices](#styled-components-best-practices)
  ...
  - [Single/Multiple Styled Components](#singlemultiple-styled-components)
...
- [`addEventListener` support abort](#addeventlistener-support-abort)
- [Installing react-native-unimodules](#installing-react-native-unimodules)
- [Using Libraries in React Native](#using-libraries-in-react-native)
  - [Linking Native Code on iOS](#linking-native-code-on-ios)
  - [Linking Native Code on Android](#linking-native-code-on-android)
- [`$$` in DevTools](#-in-devtools)
- [Building React Components Using Children Props and Context API](#building-react-components-using-children-props-and-context-api)
...
- [Tips from React Native document](#tips-from-react-native-document)
  - [Resize mode for `<Image>`](#resize-mode-for-image)
  - [Default values of `FlexBox` in React Native](#default-values-of-flexbox-in-react-native)
  - [`style` vs `contentContainerStyle` in `ScrollView`](#style-vs-contentcontainerstyle-in-scrollview)
  - [On `<ScrollView />`](#on-scrollview-)
  - [Use `contentContainerStyle` in customer component](#use-contentcontainerstyle-in-customer-component)
  - [Styling on `<View />`](#styling-on-view-)
- [`useRef` and `setTimeout` in TypeScript](#useref-and-settimeout-in-typescript)
- [Record screen in iOS Simulator](#record-screen-in-ios-simulator)
- [`onBlur` and `onFocus` issues of `TextInput`](#onblur-and-onfocus-issues-of-textinput)
- [Query keys from React Query](#query-keys-from-react-query)
- [`React.memo` hint](#reactmemo-hint)
- [Destructuring Object conditionally](#destructuring-object-conditionally)
- [Define `Enums` in JSDoc](#define-enums-in-jsdoc)
- [Get key types from `Enum`](#get-key-types-from-enum)
- [Default parameter in ES6](#default-parameter-in-es6)
- [Styled-Components features not support in React Native](#styled-components-features-not-support-in-react-native)
- [Styled Props tips in React Native](#styled-props-tips-in-react-native)
- [Use React Native build-in style types](#use-react-native-build-in-style-types)
- [Styled-components for React Native](#styled-components-for-react-native)
    - [`attrs` method DO NOT support `style` props](#attrs-method-do-not-support-style-props)
    - [How to define additional props](#how-to-define-additional-props)
- [customize component display name](#customize-component-display-name)
- [Default `backgroundColor` for `<View />` is `#FAFAFA`](#default-backgroundcolor-for-view--is-fafafa)
- [A code style](#a-code-style)
- [How to use fonts in ReactNative](#how-to-use-fonts-in-reactnative)
- [non-null assertion operator (the postfix ! character).](#non-null-assertion-operator-the-postfix--character)
- [Fix issues on iOS simulator](#fix-issues-on-ios-simulator)
  - [Reset simulator](#reset-simulator)
  - [Activate keyboard](#activate-keyboard)
- [How to access dot file or folder in MacOS](#how-to-access-dot-file-or-folder-in-macos)
- [Fish shell](#fish-shell)
- [Cold boot virtual android device in Android Studio](#cold-boot-virtual-android-device-in-android-studio)
- [Use 'open' command to open a file with its default browser.](#use-open-command-to-open-a-file-with-its-default-browser)
- [How to dark mode](#how-to-dark-mode)
- [`package.json`version](#packagejsonversion)
- [Create React App with TypeScript issue](#create-react-app-with-typescript-issue)
- [How to rename Git branch](#how-to-rename-git-branch)
- [Add environment variables to CodeSandbox](#add-environment-variables-to-codesandbox)
- [ContextAPI Initiative in twilio-video-app-react](#contextapi-initiative-in-twilio-video-app-react)
- [React native tool port](#react-native-tool-port)
- [Array.prototype.slice](#arrayprototypeslice)
- [Debug practices](#debug-practices)
- [How to Update Angular](#how-to-update-angular)
- [可观察对象的退订](#可观察对象的退订)
- [Disabled type check temporarily](#disabled-type-check-temporarily)
- [DOM 事件的行内回调形式](#dom-事件的行内回调形式)
- [Prettier End of Line](#prettier-end-of-line)
- [Safe area to display in Expo](#safe-area-to-display-in-expo)
- [Git rebase](#git-rebase)
- [Remove a file from Git commit](#remove-a-file-from-git-commit)
- [Make `yarn golbal add` works](#make-yarn-golbal-add-works)
- [`clamp()` function in CSS](#clamp-function-in-css)
- [How Array sort works in JavaScript](#how-array-sort-works-in-javascript)
- [How to await for multiple results in parallel](#how-to-await-for-multiple-results-in-parallel)
- [Set `const` variable as `readonly`](#set-const-variable-as-readonly)
- [Catch error in `async / await`](#catch-error-in-async--await)
- [Make sure property exist](#make-sure-property-exist)
- [Initiate `tsconfig.json`](#initiate-tsconfigjson)
- [Unit test resources](#unit-test-resources)
- [Testing React components that update asynchronously with React Testing Library](#testing-react-components-that-update-asynchronously-with-react-testing-library)
- [Avoid unnecessary function invoking](#avoid-unnecessary-function-invoking)
- [Multifolder in VS Code](#multifolder-in-vs-code)
